...
판독 시작 조건은 「디코드를 시도하는 자리」를 정할 뿐이고, 판독 성립은 기대 바코드가 실제로 읽혀야 합니다. 채점의 판독 항목(Sub 2-2)은 읽힌 순간에 상품이 들려 있고 빔 출발선에서 상품 표면까지 18 cm 이내여야 인정합니다. 스캐너 카메라는 800×500, 초점거리 31.43 mm, 조리개 27.24 × 17.03 mm (화각 가로 46.9도 · 세로 30.3도, 2026-09-18 에 30% 넓힘)입니다. 계산대 빨간 테두리와 스캐너의 초록 LED 도 스캐너 카메라가 기대 코드를 읽은 순간에만 켜집니다(10초 유지). 조준 항목(Sub 2-1)의 15 cm · 1초 연속 조건은 그대로입니다.
