...
동봉된 6편 말고, 주최 측이 공개한 학습 데이터도 틀 수 있습니다 — `taskC/release1/` 1,089편, `taskC/release2/` 560편(품목당 70편, 2026-09-20 추가).
...
```bash
cd /workspace/cyclo_lab
${ISAACLAB_PATH}/_isaac_sim/python.sh -u     /workspace/challenge_scripts/task_c_replay.py     --lerobot /path/to/2026CS-Store-Challenge/taskC/release1 --episode-index 792
```

`--lerobot` 에는 `taskC/release1` 또는 `taskC/release2` 폴더(`meta/` 와 `data/` 가 있는 곳)를 줍니다.
편 번호는 그 폴더의 `meta/taskC_episodes.jsonl` 에 있는 `episode_index` 입니다. 그 파일이
편마다 상품·바코드·등급과 함께 **장면 시드(`seed`)** 를 알려 줍니다.

**장면은 데이터셋이 아니라 이 저장소가 싣습니다.** release1 1,089편의 계산대 배치는
`scripts/taskC/scenes/<시드>.json` 에, release2 560편은 `scripts/taskC/scenes/release2/<품목>_<시드>.json` 에
있고, 재생기가 위 `seed`(release2 는 `source`)로 짝을 찾아 장면을 세운 뒤
...
스캐너 조준(Sub 2-1)은 **들려 있음 · 화면 점유 10% 이상 · 빔에서 0.15 m 이내**를 0.5초 연속 유지하면 성립합니다
(2026-09-18: 1.0초 → 0.5초). QR 판독(Sub 2-2)이 성립하면 조준도 함께 인정합니다 — 판독은 같은 세 조건을 만족한
프레임에서 카메라가 기대 바코드를 실제로 읽어야 하므로 조준보다 엄격하기 때문입니다.
...
| 단계 | 동작 |
|---|---|
| 판독 시작 | 스캐너의 빨간 빔이 대상 상품에 닿아 있고, 빔 출발선에서 QR 타일까지의 직선거리가 18 cm 이내인 프레임마다 디코드를 시도합니다 (`TASKC_QR_GATE_DMAX_MM`, 기본 180). 횡이탈·면각·화각 조건은 없습니다 |
| 이미지 판독 | 그 프레임의 스캐너 카메라 그림을 렌더해 디코드합니다 |
| 인식 후 | 기대 코드가 읽히면 1초만 더 읽고 판독기를 끕니다 (`TASKC_QR_KEEP_S`). 빔이 상품에서 떨어졌다가 다시 닿거나 대상 상품이 바뀌면 다시 켜집니다 |

판독 시작 조건은 「디코드를 시도하는 자리」를 정할 뿐이고, 판독 성립은 기대 바코드가 실제로 읽혀야 합니다. 채점의 판독 항목(Sub 2-2)은 읽힌 순간에 상품이 들려 있고 빔 출발선에서 상품 표면까지 18 cm 이내여야 인정합니다. 스캐너 카메라는 800×500, 초점거리 31.43 mm, 조리개 24.10 × 15.06 mm (화각 가로 42.0도 · 세로 26.9도, 원본 20.955 × 13.097 mm 보다 15% 넓힘 — 2026-09-18 의 30% 에서 2026-09-26 에 줄임)입니다. 계산대 빨간 테두리와 스캐너의 초록 LED 도 스캐너 카메라가 기대 코드를 읽은 순간에만 켜집니다(10초 유지). 조준 항목(Sub 2-1)의 15 cm · 1초 연속 조건은 그대로입니다.

`TASKC_QR_IMG=0`으로 끄면 판독 자체를 하지 않습니다 — 인식 표시도 켜지지 않고 판독·배치 점수도 없습니다. 기하 조건만으로 인식을 인정하는 방식은 데이터 수집 파이프라인에만 있고, 재생기·씬 생성기·채점기에는 없습니다.
...
| | 프링글스 사워크림 (헛집음: 79mm 만 들려 스캐너까지 0.61 m) | 2 | 2 | 0 | 0 | 0 | 4 |
| | 예감 | 2 | 2 | 3 | 7 | 3 | 17 |
| | **총점** | | | | | | **38 / 51** |
