...
def score(m, th=None):
    ...
    # ALL 1# (`no_hit`) 은 **`held` 뒤에서** 판정한다 -- 도착·들고 있음에 묶였기 때문이다.
    # 출력 순서는 `ITEMS` 가 정하므로 여기서 판정 순서를 바꿔도 보고는 그대로다.
    ...
    # ---- ALL 1#  가구와 부딪히지 않았는가 ----------------------------------------------------
    # [판 내내] 이므로 판이 일찍 끝났어도 **그때까지의 기록으로** 부딪혔는지를 본다.
    #
    # **그러나 부딪히지 않았다는 것만으로는 점수가 없다** (사용자 결정 2026-09-14).
    # 여섯 항목 중 이것만 「아무 일도 없으면 참」이어서, 로봇이 가만히 서 있기만 해도 판마다
    # 4 점(세 판이면 12 / 63)을 받았다 -- 2 프레임짜리 로그도 4 점이었다.  그래서 **책상 둘레
    # 구역에 바구니를 들고 도착한 판**(`arrived` 와 `held` 가 둘 다 참)만 이 4 점을 받는다.
    # 책상에 놓지 못해도 된다.  **새 문턱은 없다** -- 앞의 두 항목을 그대로 조건으로 쓴다.
    # 과제 B 의 B20(다른 상품을 안 건드렸는가)도 「놓은 판만」으로 같은 구멍을 막았다
    # (`scripts/taskB/taskb_score.py`).
    #
    # 판정 순서는 못 읽음 -> 부딪힘 -> 도착·들고 있음이다.  부딪혔으면 도착과 무관하게 실패이고,
    # 부딪히면 기록이 그 프레임에서 잘리므로 부딪힌 뒤의 도착은 애초에 성립하지 않는다.
    # 도착이나 들고 있음을 **못 읽었으면 이 항목도 None** 이다 -- 못 잰 것이 합격으로 넘어가지
    # 않게 (아래 「못 잰 것이 합격으로 넘어가지 않는가」 시험과 같은 원칙).
    #
    # **문턱이 없다.**  로봇(또는 든 바구니)의 발자국이 기물의 상자와 겹쳤는가 하나뿐이고,
    # 겹침은 0 을 넘느냐 마느냐이지 정도의 문제가 아니다.  숫자를 지어낼 자리가 없다.
    hit = furn.get("hit")
    carried = (items["arrived"]["got"], items["held"]["got"])
    if hit is None:
    ...
    elif carried == (True, True):
        items["no_hit"] = _item(True, "어느 기물과도 겹치지 않았고 바구니를 들고 책상 둘레 구역에 "
                                      "도착했다 "
                                      f"(최악 침투 {furn.get('worst_mm', 0.0):.1f} mm)")
    elif False in carried:
        items["no_hit"] = _item(False, "부딪히지는 않았으나 바구니를 들고 책상 둘레 구역에 도착한 "
                                       "적이 없어 이 점수를 받을 수 없다")
    else:
        items["no_hit"] = _item(None, "부딪히지는 않았으나 도착했는지 또는 들고 있었는지를 못 읽었다")
    ...
    if ended in ("dropped", "hit", "out_of_store"):
        ...
        if place.get("reached_desk") is False:
            ...
            #
            # 2026-09-23 부터 「쥐고 있었다」는 **집게가 물고 있었다**는 뜻이다.  집게를 편 손이
            # 얹혀 있는 것은 놓은 것으로 본다 (사용자 결정).
            items["placed"] = _item(False, "상판 높이와 자세는 맞았으나 그 순간 집게가 아직 "
                                           "바구니를 물고 있었다 — 놓아야 얹은 것이다")
        elif dm is None:
        ...
        if opened is None:
            ...
            if watch.get("hands_off") is False:
                ...
                # (2026-09-23 부터 「다시 잡았다」는 집게가 다시 물었다는 뜻이다.)
                items["stayed"] = _item(
                    False, "6초 창 안에서 집게가 바구니를 다시 물었다 — 손을 뗀 뒤 6초를 "
                ...
                # **누르다가 바구니가 밀리거나 돌면 받지 못한다** (사용자 결정 2026-09-23:
                # "누르다가 상자가 움직이면, 가점을 부여하지 않는 걸로 하자").
                #
                # 손을 얹어 두는 것은 괜찮지만(「놓았다」는 집게로 본다) 그 손으로 바구니를
                # 옮기면 「놓여 있었다」가 아니다.  값은 창 안에서 처음 제대로 앉은 뒤 밑면 네
                # 모서리 중 가장 많이 움직인 수평 거리다 (`score_from_log.py` 감시창 참조).
                # 문턱은 책상 조항과 **같은 자** `DESK_OK_MM` -- 같은 형식의 조항이다.
                #
                # `None` 은 옛 기록(필드 없음)이거나 창 안에서 한 번도 안 앉은 판이다.  뒤엣것은
                # 높이 검사가 이미 걸러내므로 여기서 따로 막지 않는다.  **NaN 은 실패로 본다**
                # -- `nan > 문턱` 이 거짓이라 그냥 두면 유리한 쪽으로 샌다.
                w_mv = watch.get("moved_mm")
                if w_mv is not None and not (w_mv <= t["DESK_OK_MM"]):
                    bad.append(f"누르다가 바구니 모서리가 {w_mv:.1f} mm 밀렸다 "
                               f"(문턱 {t['DESK_OK_MM']:.0f})")
